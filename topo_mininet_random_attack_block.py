        ...
        while not stop_event.is_set() and time.monotonic() < deadline:
            ...
            if not processes or process_ended or addresses_changed:
                if addresses_changed:
                    ...
                    _print("[EPISODE {:04d}] hping3 exited; restarting".format(episode))
                ...
                if any(process.poll() is not None for process in processes):
                    _print("[EPISODE {:04d}] hping3 failed to start; check installation".format(
                        episode
                    ))
                    _stop_processes(processes)
                    processes = []
...
def random_attack_loop(net, stop_event, seed=None):
    """Run random isolated-Mininet attacks until stop_event is set.

    Selection is balanced by model class:
      25% multi-source ICMP host flood
      25% single-source SYN host flood
      50% two-flow link flood
    """
    rng = random.Random(seed)
    episode = 1
    choices = ("icmp", "syn", "link", "link")
