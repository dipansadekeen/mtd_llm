...
import time , random
from decision_proactive import decide, load_hop_lookup, last_nonzero, PRESET_HOSTS, H1_PAIRS, log_decision_items
...
def run_shuffle(ip_manager, route_manager, i, host_count=40):
    # mode = decide(ip_manager, route_manager, i)
    # # log_decision_items(ip_manager, route_manager, i)
    # log_decision_items(ip_manager, route_manager, i, action_taken=mode)
    mode, I_ip, I_rrm = decide(ip_manager, route_manager, i)
    log_decision_items(ip_manager, route_manager, i, action_taken=mode, I_ip=I_ip, I_rrm=I_rrm)
    if mode == "ip":
        print("\nIP shuffling")

        # shuffle only preset hosts, avoid collision with non-preset hosts
        target_hosts = list(PRESET_HOSTS)
        current_ips = ip_manager.get_current_ips()

        used_ips = set()
        for host, ip in current_ips.items():
            if ip is not None and host not in target_hosts:
                used_ips.add(int(str(ip).split(".")[-1]))

        available_ips = [x for x in range(1, 255) if x not in used_ips]

        if len(available_ips) < len(target_hosts):
            print("  ⚠ Not enough free IPs for preset hosts.")
            return

        new_ips = random.sample(available_ips, len(target_hosts))
        shuffled_map = dict(zip(target_hosts, new_ips))

        ip_shuffle_endpoint(
            host=",".join(target_hosts),
            ips=",".join(str(shuffled_map[h]) for h in target_hosts),
            interval=30,
            no_block_pid=True
        )
        ...
        # also advance route history with current route values (no route change this round)
        route_candidates = [("h1", hx) for hx in PRESET_HOSTS if hx != "h1"] # may need attention
        ...
        # active option: all h1-hx pairs, hx only from PRESET_HOSTS
        route_candidates = [("h1", hx) for hx in PRESET_HOSTS if hx != "h1"]

        # option 2: keep commented for now
        # route_candidates = []
        # for i1 in range(len(PRESET_HOSTS)):
        #     for j1 in range(i1 + 1, len(PRESET_HOSTS)):
        #         route_candidates.append((PRESET_HOSTS[i1], PRESET_HOSTS[j1]))
        ...
        for a, b in route_candidates:
            ...
            # use only safe common options shared by both directions
            ...
            if not all_options:
                print(f"  ⚠ No path options found for ({a}, {b})")
                continue

            # use the actual last used value; 0 is allowed
            current_opt = hist[-1] if hist else None

            # avoid immediate reuse
            if current_opt in all_options:
                available = [opt for opt in all_options if opt != current_opt]
            else:
                available = all_options[:]

            if not available:
                print(f"  ⚠ No new path available for ({a}, {b})")
                continue

            # safest choice: lowest valid option
            # new_opt = min(available)
        ...
        if not pairs_arg:
            print("  ⚠ No valid route updates found.")
            return
        ...
        # write only the newly selected routes into history
        for (a, b), new_opt in result_map.items():
            route_manager.update_pair(a, b, new_opt)

        # also advance IP history with current IPs (no IP change this round)
        current_ips = ip_manager.get_current_ips()
        for host in all_hosts:
            last_ip = current_ips.get(host)
            if last_ip is not None:
        ...
        ip_manager.save_to_csv()
        route_manager.save_to_csv()
        print("[ROUTE SHUFFLE]", result_map)
...
i = 0
while True:
    run_shuffle(ip_manager, route_manager, i*6) # pick every 30s
    print(ip_manager.get_all_host_ips())
    i += 1
    time.sleep(30)