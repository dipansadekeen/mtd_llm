...
def send_last_octets(target_ip='192.168.10.45', port=5000):
    """
    Sends last octets to external receiver.
    Priority:
      1. ONOS learned 10.0.0.x IP
      2. fallback from HostIPQueueManager CSV/ip_history
      3. no_ip_found
    """
...
def main():
    ...
    while True:
        ...
        # One ONOS host list send per cycle (not per host)
        try:
            send_last_octets('192.168.10.45', 5000)
        except Exception as e:
            print(f"[!] send_last_octets failed: {e}")
