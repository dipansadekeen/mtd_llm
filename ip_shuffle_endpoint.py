...
class IPHopper:
    ...
    def __init__(self):
        ...
        self.anchor_ready = threading.Event()
        self.anchor_ready.set()
    ...
    def run_host_pipeline(self, host, ip, cidr, ipfile, no_block_pid):
        """Run one host's dependent steps in order; called concurrently per host."""
        st = self.ensure_host_state(host, no_block_pid=no_block_pid)
        if not st:
            if host == "h1":
                self.anchor_ready.set()
            return False
        ...
        pid = st["pid"]
        iface = st["iface"]
        ipfile_path = self.ipfile_for_host(ipfile, host)

        if not self.set_ip_and_verify(pid, iface, ip, cidr):
            print(f"[-] {host}: IP set/verify failed for {ip}")
            if host == "h1":
                self.anchor_ready.set()
            return False

        if host == "h1":
            # Other hosts use h1 as the ONOS-learning anchor. Release them only
            # after h1 has a verified address, while all earlier work stays parallel.
            self.anchor_ready.set()

        # self.atomic_write(ipfile_path, ip)
        print(f"[WROTE] {ipfile_path} <- {ip}")
        self.reapply_nat_and_routes_if_needed(host, pid, iface, ip)
        self.host_remove(host)
        if host != "h1":
            self.anchor_ready.wait(timeout=30)
        self.learn_onos(host, ip)
        return True
    ...
    def run_once(
        ...
        if len(hosts) != len(set(hosts)):
            raise ValueError("Duplicate hosts are not allowed in parallel mode")
        ...
        # Select every destination before starting threads. This prevents two
        # parallel workers from randomly choosing the same address.
        assignments = {}
        reserved = set(avoid_set)
        for h in hosts:
            if host_ip_map:
                ip = host_ip_map[h]
                if ip in avoid_set:
                    raise ValueError(f"{h}: mapped IP {ip} is in avoid list")
                if ip in assignments.values():
                    raise ValueError(f"Duplicate mapped IP is not allowed: {ip}")
            else:
                if ips_list:
                    available = [candidate for candidate in ips_list if candidate not in reserved]
                    if not available:
                        raise ValueError("Not enough distinct, non-avoided IPs for all hosts")
                    ip = random.choice(available)
                    if ip in avoid_set:
                        raise ValueError(f"{h}: chosen IP {ip} is in avoid list")
                else:
                    ip = self.rand_ip_from_list(low, high, reserved)
            assignments[h] = ip
            reserved.add(ip)

        # Default: one worker per host, so all requested hosts can progress at once.
        workers = len(hosts) if max_workers is None else min(max_workers, len(hosts))
        if workers < 1:
            raise ValueError("max_workers must be at least 1")

        if "h1" in hosts:
            self.anchor_ready.clear()
        else:
            self.anchor_ready.set()

        results = {}
        with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="ip-hop") as pool:
            future_to_host = {
                pool.submit(
                    self.run_host_pipeline,
                    h,
                    assignments[h],
                    cidr,
                    ipfile,
            ...
            for future in as_completed(future_to_host):
                ...
                try:
                    ...
                    if h == "h1":
                        self.anchor_ready.set()
                    print(f"[!] {h}: parallel pipeline failed: {e}")

        succeeded = sum(bool(results.get(h)) for h in hosts)
        print(f"[PARALLEL] completed {succeeded}/{len(hosts)} host pipelines")
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="parallel host workers (default: one per requested host)",
    )
    args = ap.parse_args()
