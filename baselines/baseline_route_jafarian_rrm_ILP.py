...
# ============================================================
def select_routes_ilp(
    df,
    active_pairs,
    recent_route_map,
    link_loads,
    max_hops=6,
    load_threshold_mbps=10,
    ...
    """
    ILP-based Jafarian-style route selection.

    Selects one route option per active flow/pair jointly.
    """
    active_pairs = active_pairs[:pair_count_per_cycle]

    candidates = []

    for pair in active_pairs:
        pair_df = df[df["pair_key"] == pair].copy()

        for idx, row in pair_df.iterrows():
            if max_hops is not None and int(row["hop_count"]) > max_hops:
                continue
            ...
            if load_threshold_mbps is not None:
                if any(load >= float(load_threshold_mbps) for load in path_loads):
                    continue

            candidates.append((pair, idx, row))

    if not candidates:
        print("[JAFARIAN-ILP] No feasible route candidates.")
        return []

    prob = pulp.LpProblem("Jafarian_ILP_RRM", pulp.LpMinimize)

    x = {}
    for pair, idx, row in candidates:
        x[(pair, idx)] = pulp.LpVariable(
            f"x_{pair[0]}_{pair[1]}_{idx}",
            cat="Binary"
        )

    # Constraint 1: choose exactly one option per active pair if feasible
    feasible_pairs = sorted(set(pair for pair, _, _ in candidates))

    for pair in feasible_pairs:
        pair_vars = [
            x[(p, idx)]
            for p, idx, _ in candidates
            if p == pair
        ]
        prob += pulp.lpSum(pair_vars) == 1

    # Constraint 2: link capacity / overlap control
    all_links = set()
    ...
    for lk in all_links:
        vars_using_link = [
            x[(pair, idx)]
            for pair, idx, row in candidates
            if lk in row["norm_links"]
        ]
        prob += pulp.lpSum(vars_using_link) <= link_capacity

    # Objective: minimize load + reuse + hop count
    objective_terms = []
    ...
    for pair, idx, row in candidates:
        ...
        recent = recent_route_map.get(pair)
        reuse_penalty = 1 if recent is not None and str(option_number) == str(recent) else 0

        cost = (
            w_load * max_path_load
            + w_reuse * reuse_penalty
            + w_hop * hop_count
        )

        objective_terms.append(cost * x[(pair, idx)])
    ...
    if pulp.LpStatus[status] != "Optimal":
        print(f"[JAFARIAN-ILP] Solver status: {pulp.LpStatus[status]}")
        return []

    selected = []

    for pair, idx, row in candidates:
        if pulp.value(x[(pair, idx)]) == 1:
            selected.append({
                "host1": str(row["host1"]),
                "host2": str(row["host2"]),
                "option_number": int(row["option_number"])
            ...
            print(
                f"[JAFARIAN-ILP] selected {row['host1']}<->{row['host2']} "
                f"opt={int(row['option_number'])} "
                f"hops={int(row['hop_count'])} "
                f"links={row['norm_links']}"
            )
...
def select_jafarian_candidates(
    ...
    print("\n[JAFARIAN-ILP] Active-flow ILP Efficient RRM baseline")
    print(f"[JAFARIAN-ILP] candidate_pairs={len(protected_pairs)}")
    print(f"[JAFARIAN-ILP] pair_count_per_cycle={pair_count_per_cycle}")
    print(f"[JAFARIAN-ILP] max_hops={max_hops}")
    print(f"[JAFARIAN-ILP] load_threshold_mbps={load_threshold_mbps}")

    selected = select_routes_ilp(
        df=df,
        active_pairs=protected_pairs,
        recent_route_map=recent_route_map,
        link_loads=link_loads,
        max_hops=max_hops,
        load_threshold_mbps=load_threshold_mbps,
        pair_count_per_cycle=pair_count_per_cycle,
        link_capacity=1
    )

    print(f"[JAFARIAN-ILP] selected={len(selected)} candidates")
    return selected
