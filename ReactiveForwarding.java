...
public class ReactiveForwarding {
    ...
    // My Change
    private String labelDevice(DeviceId did) {
        Device d = deviceService.getDevice(did);
        if (d == null) return did.toString();
        String name = d.annotations().value(AnnotationKeys.NAME);
        return (name != null && !name.isEmpty()) ? name : did.toString();
    }

    private String labelHost(Host h) {
        if (h == null) return "(unknown-host)";
        String hName = h.annotations().value(AnnotationKeys.NAME);
        if (hName != null && !hName.isEmpty()) return hName;
        // fallbacks: IP or MAC
        if (!h.ipAddresses().isEmpty()) return h.ipAddresses().iterator().next().toString();
        return h.mac().toString();
    }

    private void logPrettyPath(PacketContext ctx, Path path) {
        // find src/dst hosts from the packet
        Ethernet eth = ctx.inPacket().parsed();
        MacAddress srcMac = eth.getSourceMAC();
        MacAddress dstMac = eth.getDestinationMAC();

        Host srcHost = hostService.getHost(HostId.hostId(srcMac, VlanId.NONE));
        Host dstHost = hostService.getHost(HostId.hostId(dstMac, VlanId.NONE));

        StringBuilder chain = new StringBuilder();

        // start: source host
        chain.append(labelHost(srcHost));

        // then: devices along the path (unique sequence of switches)
        DeviceId current = path.src().deviceId();
        chain.append(" -> ").append(labelDevice(current));
        for (Link l : path.links()) {
            DeviceId next = l.dst().deviceId();
            if (!next.equals(current)) {
                chain.append(" -> ").append(labelDevice(next));
                current = next;
            }
        }

        // end: destination host
        chain.append(" -> ").append(labelHost(dstHost));

        log.info("PING path ({} hops): {}", path.links().size(), chain.toString());
    }
    // My change ends here

    //myedit 2
    // path to your flag file
    private static final String FLAG_FILE = "/tmp/shuffle_flag.txt";

    // Fast, in-process flag reader: returns 0 or 1 (defaults to 0 on error)
    private int readFlag() {
        try {
            String s = new String(Files.readAllBytes(Paths.get(FLAG_FILE))).trim();
            return (s.length() > 0 && s.charAt(0) == '1') ? 1 : 0;
        } catch (Exception e) {
            log.debug("Flag read failed (defaulting to 0): {}", e.toString());
            return 0;
        }
    }
    ...
    //myedit 3 || read index or jflag
    // Generalized version that takes a file path
    private int readAnyFlag(String filePath) {
        try {
            String s = new String(Files.readAllBytes(Paths.get(filePath))).trim();
            return Integer.parseInt(s);  // Parse the entire string
        } catch (Exception e) {
            log.debug("Index read failed for {} (defaulting to 0): {}", filePath, e.toString());
            return 0;
        }
    }
    // myedit 3 ends
    ...
    //myedit 3 ||trying to bringout the path logics
    // Keep only this inner class to hold path analysis result
    public static class PathAnalysisResult {
        public MacAddress srcMac;
        public MacAddress dstMac;
        public String srcIp;
        public String dstIp;
        public int flag;  // 1 if ≥3 distinct weights, else 0
    }

    // private PathAnalysisResult pathAnalysis(PacketContext context, Set<Path> paths) {
    ...
    //     if (ipv4Pkt == null) return result;  // additional null check

    //     result.srcIp = Ip4Address.valueOf(ipv4Pkt.getSourceAddress()).toString();
    //     result.dstIp = Ip4Address.valueOf(ipv4Pkt.getDestinationAddress()).toString();

    //     result.srcMac = ethPkt.getSourceMAC();
    //     result.dstMac = ethPkt.getDestinationMAC();

    //     // Collect all scalar weights from paths
    //     Set<Double> uniqueWeights = new HashSet<>();
    //     for (Path path : paths) {
    //         double weight = path.cost();  // Scalar weight as double
    //         uniqueWeights.add(weight);
    //     }

    //     result.flag = (uniqueWeights.size() >= 3) ? 1 : 0;
    //     return result;
    // }


    //myedit 4 || trying to allow arp
    private PathAnalysisResult pathAnalysis(PacketContext context, Set<Path> paths) {
    PathAnalysisResult result = new PathAnalysisResult();

    Ethernet ethPkt = context.inPacket().parsed();
    if (ethPkt == null) return result; // null check only

    result.srcMac = ethPkt.getSourceMAC();
    result.dstMac = ethPkt.getDestinationMAC();

    // Optional: only process IPs if it's IPv4
    ...
    // Count unique path costs (works for any type)
    Set<Double> uniqueWeights = new HashSet<>();
    for (Path path : paths) {
        uniqueWeights.add(path.cost());
    }
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            */
            LinkWeigher hopCountWeigher = new LinkWeigher() {
                @Override
                public Weight weight(TopologyEdge edge) {
                    return ScalarWeight.toWeight(1); // each hop costs 1
                }

                @Override
                public Weight getInitialWeight() {
                    return ScalarWeight.toWeight(0); // starting weight
                }

                @Override
                public Weight getNonViableWeight() {
                    return ScalarWeight.NON_VIABLE_WEIGHT; // unreachable link
                }
            };

            //myedit 4 || reducing cost when nonrrm in action
            // read the current RRM flag
            int rrm_flag_2 = readFlag();

            // dynamically select k value
            int kValue = (rrm_flag_2 == 1) ? 10 : 1;

            log.info("RRM flag: {}, using kValue = {}", rrm_flag_2, kValue);
            //my edit 4 ends

            Set<Path> paths =
                    topologyService.getKShortestPaths(topologyService.currentTopology(),
                                             pkt.receivedFrom().deviceId(),
                                             dst.location().deviceId(),
                                             hopCountWeigher,
                                            //  3);//my edit 2
            ...
            log.info("\n\n");
            log.warn("paths count is: {}", paths.size());
            // for (Path path : paths) {
            //     log.info(path.toString());// myedit2 uncomment later
            //     log.info("----------------------------------");
            /*
                log.info(path.src().toString());
                log.info("Path with {} hops: {}", path.links().size(), path);

                // Optional: print hop-by-hop details
                path.links().forEach(link ->
                    log.info("  {}:{} -> {}:{}",
                            link.src().deviceId(), link.src().port(),
                            link.dst().deviceId(), link.dst().port())
                );
                Device s = ReactiveForwarding.this.deviceService.getDevice(path.src().deviceId());
                Device d = ReactiveForwarding.this.deviceService.getDevice(path.dst().deviceId());
                log.warn("s is {}",s);
                log.warn("d is {}", d);
                //String sName = (s != null) ? s.annotations().value("name") : null;
                //String dName = (d != null) ? d.annotations().value("name") : null;

                //log.info("PING path {} hops: {} ({}) -> {} ({})",
                //        path.links().size(), path.src(), sName, path.dst(), dName);
                */
            
            // } //myedit 2 |included with possible path printing
            log.warn("==========");
            /*
            //ConnectPoint srcCp = paths.src();            // source connect point of the path
            //DeviceId srcDev    = paths.src().deviceId();      // e.g., of:0000000000000001
            //PortNumber srcPort = srcCp.port();          // e.g., 1

            //Device d = DeviceService.getDevice(srcDev); // inject DeviceService
            
            String alias = (DeviceService.getDevice(paths.src().deviceId()) != null) ? DeviceService.getDevice(paths.src().deviceId()).annotations().value("name") : null;

            //log.info("Path src: deviceId={}, port={}, alias={}", srcDev, srcPort, alias);
            ...
            */
            ...
            //myedit 3 ||log mac and ip
            // Result analysis = pathAnalysis(context, paths);
            PathAnalysisResult analysis = pathAnalysis(context, paths);
            // if (analysis.srcMac !=null){
            log.info("[FlowAnalysis] MACs: {} → {}, IPs: {} → {}, Distinct Path Weights ≥ 3? → {}",analysis.srcMac, analysis.dstMac, analysis.srcIp, analysis.dstIp, analysis.flag);
            // }
            //myedit3 ends


            // myedit 3 | trying to print the paths and match with my target
            // for (Path p : paths) {
            //     log.info("Path:");
            ...
            //             link.dst().deviceId(), link.dst().port());
            //     }
            //     log.info("Hop count: {}", p.links().size());
            //     log.info("Cost: {}", p.cost());
            //     log.info("==========");
            // }
            //myedit 3 ends
            //myedit 3 || match the destined path I desired
            // Example target strings (you can load from file too)
            // Define the target file (the .txt file you prepared with expected paths)


            int rrm_flag = readFlag();
            //flag 0 default, flag 1 randomized
            if (rrm_flag==1){
            log.info("RRM FLAG is found: {}", rrm_flag);

                // filtering arp packets || remove this if needed to read arp as well
                if(analysis.srcMac != null){
                String targetFile = "/tmp/path_match_log.txt";
                // Read all lines from file into a Set
                Set<String> targetLines = new HashSet<>();
                try {
                    targetLines.addAll(Files.readAllLines(Paths.get(targetFile), StandardCharsets.UTF_8));
                    log.info("Loaded {} target paths from {}", targetLines.size(), targetFile);
                } catch (IOException e) {
                    log.error("Could not read target file {}", targetFile, e);
                }

                // For each candidate path from ONOS
                int i=0;
                for (Path p : paths) {
                    // Build hop strings (like ONOS prints them)
                    List<String> hopStrings = new ArrayList<>();
                    for (Link link : p.links()) {
                        hopStrings.add(link.src().deviceId() + ":" + link.src().port()
                                    + " -> " +
                                    link.dst().deviceId() + ":" + link.dst().port());
                        
                    }
                    String hopsJoined = String.join(", ", hopStrings);

                    // Candidate line in the same format as the file
                    String candidate = String.format("%s, %s, %s, %.1f",
                        analysis.srcMac,
                        analysis.dstMac,
                        hopsJoined,
                        p.cost()
                    ...
                    // Compare against the file contents
                    int flag = targetLines.contains(candidate) ? (int)p.cost() : 0;

                    // Log the result
                    log.info("[PathMatch] Candidate={} | Flag={}, i: {}", candidate, flag,i);
                    

                    // // myedit 4 | logging the paths
                    // // Build log line
                    // String line = String.format("[PathMatch] Candidate=%s | Flag=%d | i=%d%n", candidate, flag, i);

                    // // Log normally to ONOS console
                    // log.info(line.trim());

                    // // Also append to external text file
                    // try (FileWriter fw = new FileWriter("/tmp/pathmatch_log.txt", true)) {
                    //     fw.write(line);
                    // } catch (IOException e) {
                    //     log.warn("Failed to write to /tmp/pathmatch_log.txt: {}", e.toString());
                    // }
                    // // myedit 4 ends



                    // myedit 4 || trying to get paths without the redundant immediate hop ones || same as upper one but with more selectivity
                    // Define logical switch–host MAC connections (edit as needed)
                    // Logical map: switch (of:ID) → host MAC connected to it
                    Map<String, List<String>> switchMacMap = Map.ofEntries(
                        Map.entry("of:0000000000000001", List.of(
                            "00:00:00:00:00:01")),
                        Map.entry("of:0000000000000002", List.of(
                        ...
                        Map.entry("of:0000000000000003", List.of(
                            "00:00:00:00:00:09", "00:00:00:00:00:0A",
                            "00:00:00:00:00:0B", "00:00:00:00:00:0C", "00:00:00:00:00:0D",
                            "00:00:00:00:00:0E", "00:00:00:00:00:0F")),
                        Map.entry("of:0000000000000004", List.of(
                            "00:00:00:00:00:10", "00:00:00:00:00:11",
                            "00:00:00:00:00:12", "00:00:00:00:00:13", "00:00:00:00:00:14",
                            "00:00:00:00:00:15")),
                        Map.entry("of:0000000000000005", List.of(
                            "00:00:00:00:00:16", "00:00:00:00:00:17", "00:00:00:00:00:18",
                            "00:00:00:00:00:19", "00:00:00:00:00:1A", "00:00:00:00:00:1B")),
                        Map.entry("of:0000000000000006", List.of(
                            "00:00:00:00:00:1C", "00:00:00:00:00:1D", "00:00:00:00:00:1E",
                            "00:00:00:00:00:1F", "00:00:00:00:00:20", "00:00:00:00:00:21")),
                        Map.entry("of:0000000000000007", List.of(
                            "00:00:00:00:00:22", "00:00:00:00:00:23", "00:00:00:00:00:24",
                            "00:00:00:00:00:25", "00:00:00:00:00:26", "00:00:00:00:00:27","00:00:00:00:00:28"))
                    );

                    String srcSwitch = context.inPacket().receivedFrom().deviceId().toString();
                    String srcMac    = context.inPacket().parsed().getSourceMAC().toString();

                    // Check if that MAC logically belongs under the switch
                    boolean isConnected = switchMacMap.containsKey(srcSwitch) &&
                                        switchMacMap.get(srcSwitch).contains(srcMac);

                    if (isConnected) {
                        String line = String.format("[PathMatch] Candidate=%s | Flag=%d | i=%d%n", candidate, flag, i);
                        log.info(line.trim());

                        if (readAnyFlag("/tmp/pathlog_flag.txt") == 1) {
                            try (FileWriter fw = new FileWriter("/tmp/pathmatch_log.txt", true)) {
                                fw.write(line);
                            } catch (IOException e) {
                                log.warn("Failed to write to /tmp/pathmatch_log.txt: {}", e.toString());
                        ...
                        log.warn("[PathMatch] Skipped invalid mapping: {} ≠ {}", srcSwitch, srcMac);
                    }
                    // myedit 4 ends
                    ...
                    // if flag != 0
                    if (flag != 0) {
                        paths = Collections.singleton(p); // create a new Set containing just p
                        break;
                    }
            ...
            // came from; if no such path, flood and bail. //commented original code myedit 2//
            Path path = pickForwardPathIfPossible(paths, pkt.receivedFrom().port()); 
            //commented original code ends myedit 2//


            logPrettyPath(context, path);   // prints the path taken
            if (path == null) {
            ...
            //myedit 3 || logging the type of packets installed
            if (ethPkt.getEtherType() == Ethernet.TYPE_ARP) {
                log.info("[FLOW_INSTALL] Installing ARP forwarding rule.");
            } else if (ethPkt.getEtherType() == Ethernet.TYPE_IPV4) {
                IPv4 ipv4 = (IPv4) ethPkt.getPayload();
                if (ipv4.getProtocol() == IPv4.PROTOCOL_ICMP) {
                    log.info("[FLOW_INSTALL] Installing ICMP (ping) rule: {} -> {}",
                        Ip4Prefix.valueOf(ipv4.getSourceAddress(), 32),
                        Ip4Prefix.valueOf(ipv4.getDestinationAddress(), 32));
                } else {
                    log.info("[FLOW_INSTALL] Installing general IPv4 rule (protocol={}): {} -> {}",
                        ipv4.getProtocol(),
                        Ip4Prefix.valueOf(ipv4.getSourceAddress(), 32),
                        Ip4Prefix.valueOf(ipv4.getDestinationAddress(), 32));
                }
            } else {
                log.info("[FLOW_INSTALL] Installing rule for ethertype: 0x{}",
                    Integer.toHexString(ethPkt.getEtherType() & 0xffff));
            }
            //myedit 3 ends
            ...
            // log.info("installed path, {}",path);//myedit 2
        }
...
//new flag ops
    // private Path pickForwardPathIfPossible(Set<Path> paths, PortNumber notToPort) {
    //     // // Count how many valid paths exist

    //     int flag = readFlag();
    //     log.info("RRM FLAG (read): {}", flag);
    ...
    //     if (validCount == 0) {
    //         return null; // no eligible path
    //     }

    //     // Pick random index among valid paths
    //     Random rand = new Random();
    //     // Random rand = new Random(42); //doesn't work
    //     int targetIndex = rand.nextInt(validCount);
    //     // Iterate until reaching the random index
    //     int currentIndex = 0;
    //     for (Path path : paths) {
    //         if (!path.src().port().equals(notToPort)) {
    //             if (currentIndex ==targetIndex) 
    //             {
    //                 // log.info("The targetIndex is ->{} and selected path is-> {} ",targetIndex, path);
    //                 // log.info("\n\nThe currentIndex is :{} || targetIndex is ->{} and selected path is-> {} \n\n",currentIndex,targetIndex, path);
    //                 return path;
    //             }
    //             currentIndex++;
    //         }
    ...
    private void backTrackBadNodes(Set<Path> shortestPaths, DeviceId dstId, SrcDstPair sd) {
        //myedit 2
        log.info("accessed backTrackBadNodes: {} |--| {} |--| {} ", shortestPaths,dstId,sd);
        //myedit 2 ends
