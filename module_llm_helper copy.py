...
# new | take k steps 
def wait_for_next_k_ts(_prepare, csv_file, decision_ts, k=3):
    while True:
...
def validate_effect(scene, final_obj, decision_ts, host_csv=HOST_CSV, link_csv=LINK_CSV):
    ...
    next_two_host = wait_for_next_k_ts(_prepare, host_csv, decision_ts,k=3)
    next_two_link = wait_for_next_k_ts(_prepare, link_csv, decision_ts,k=3)
    ...
    for macu in other_candidate_macs:
        ...
        #     weighted_mean(cand["rx_kbps_trend"])
        # ) / 1000.0
        # not total but maximum of one.        # not total but maximum of one.
        before_mbps = float(
            max(
                weighted_mean(cand["tx_kbps_trend"]),
                weighted_mean(cand["rx_kbps_trend"])
